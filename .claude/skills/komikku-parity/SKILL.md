...
| — | Backlog complete | Done | All 8 tracked areas have been diffed against Komikku — each is either fixed to parity, confirmed already at parity, or confirmed genuinely Otaku-exclusive with no Komikku equivalent to diff against. This does **not** mean every gap is closed: real, deferred parity gaps remain (Statistics downloadCount/Trackers section, Settings selective-backup/Advanced-screen/several small toggles, Browse custom-cover migration, Update Errors dedicated screen + More-tab auto-open, Onboarding storage step + install-unknown-apps permission) and are tracked in issue #1192 for future follow-up — see that issue for the full list. |

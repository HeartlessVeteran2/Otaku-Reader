...
| 8 | More / stats / remaining screens | In progress (PR pending) | More tab: fixed (PR #1190) — live download-queue subtitle via `DownloadQueueDisplayState`. Stats screen: fixed `completedMangaCount` and `totalChapterCount` gaps — `ReadingStats` now carries both, sourced from `MangaDao.getCompletedMangaCount()` (pre-existing query) and a new `ChapterDao.getTotalChapterCountForLibrary()` query; displayed in a new `LibraryDetailCard` on `StatisticsScreen` below the hero card. Still not built: `downloadCount` (no existing query — downloads aren't tracked as a `chapters` column, would need a query against the download-queue's own table) and the entire Trackers section (tracked-title count / mean score / tracker count — `TrackRepository` only has `observeEntriesForManga(mangaId)`, no cross-manga "all entries" query yet; would need a new DAO method). Remaining screens not yet diffed: Onboarding, About, Update Errors, QR Library Share/Scan. |

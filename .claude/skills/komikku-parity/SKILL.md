...
- **Komikku's "Advanced" settings screen has no Otaku equivalent at all** — clear database, clear
  ...
  extension-installer choice (System/Shizuku/Private). **Still deferred**, spun out to its own
  issue when #1192 closed — none of these have supporting infrastructure in Otaku today (no
  cookie jar is even configured on the shared OkHttpClient — it's `CookieJar.NO_COOKIES` by
  default); building the screen means building the infrastructure first.
- ~~Reader: no `removeAfterReadSlots`~~ ✅ Resolved in #1203. ~~Library: no charging/metered
  auto-update restrictions~~, ~~Tracking: no `trackOnAddingToLibrary`/`autoUpdateTrackOnMarkRead`
  opt-outs~~, ~~Security: no `hideNotificationContent`~~ ✅ all resolved in #1202
  (`trackOnAddingToLibrary` turned out N/A — Otaku has no auto-track-on-add behavior to opt out
  of). ~~Onboarding: no storage-location step~~ ✅ Resolved in #1204 (install-unknown-apps item
  skipped as N/A — Otaku's `ExtensionInstaller` never launches the system installer).
  ~~Update Errors: flat dialog instead of a dedicated screen~~ ✅ Resolved in #1205.
  ~~Browse: migrate has no custom-cover migration or configurable options~~ ✅ Resolved in #1206
  (`MigrationFlag` toggle set + net-new `MangaRepository.copyCustomCover`).

One new small gap surfaced while building #1206: migration doesn't carry over reading-history
timestamps (readAt/duration), because `ChapterRepository` only exposes a global, unscoped
`observeHistory()` with no per-manga/per-chapter read API. Needs new repository plumbing before
it can be wired in under `MigrationFlag.CHAPTERS` — noted in #1192, not yet scheduled.

...
interface FeedDao {
    ...
    /**
     * IGNORE, not REPLACE, now that `(mangaId, chapterId)` is unique.
     *
     * REPLACE deletes the conflicting row and inserts a new one, which assigns a fresh
     * autogenerated id and resets `isRead` to false — so a chapter the user had already read in
     * the feed would silently reappear as unread every time an overlapping update re-recorded it.
     * The first write of a chapter is the one worth keeping.
     */

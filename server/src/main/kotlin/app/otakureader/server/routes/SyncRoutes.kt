    ...
    route("/sync") {
        ...
        post("/upload") {
            try {
                ...
                if (result.isSuccess) {
                    ...
                    call.application.environment.log.error("[SyncRoutes] storeSnapshot failed", cause)
                    call.respond(
                ...
                // H-9: Log the full exception server-side; return a generic message to the
                // client so that internal implementation details are never exposed.
                call.application.environment.log.error("[SyncRoutes] upload request error", e)
                call.respond(
        ...
        delete {
            ...
            if (result.isSuccess) {
                ...
                // client so that internal implementation details are never exposed.
                val cause = result.exceptionOrNull()
                call.application.environment.log.error("[SyncRoutes] deleteSnapshot failed", cause)
                call.respond(
                    HttpStatusCode.InternalServerError,

...
- Advanced (#1208): user-agent override, DNS-over-HTTPS provider, verbose network logging, clear cookies, plus the battery-optimisation shortcut and "Don't kill my app" link that previously existed only in onboarding
...
- **Never `OnConflictStrategy.REPLACE` on a table with an autogenerated key and a *secondary* unique index.** SQLite's REPLACE **deletes** the conflicting row before inserting, so `autoGenerate` hands the replacement a **new id** — and every `ON DELETE CASCADE` child of the old id goes with it. This was the same bug five times: `ChapterDao` (#1254), `MangaDao` (#1269), and — found by auditing for the rule after writing it — `TrackEntryDao`, `TrackerSyncDao.insertSyncState` and `TrackerSyncDao.insertSyncConfiguration` (#1276). All are fixed; if you add a table with an `autoGenerate` key and a secondary unique index, it is on you not to make it six. `manga` is the worse of the two, parenting eleven cascading tables, one of which (`chapters`) parents three more — so a single conflicting insert could take the chapter list, every read marker, page bookmark and chapter note, both tracker tables, categories, reading-list membership and the AniList link. Both were reachable only through a race, because every caller does a read-then-act "look it up, insert if absent" that holds nothing across the two steps (checklist item 2).
...
**One cause of those 403s has since been removed, and the sweep predates the fix.** Until #1285 the shared
client had no cookie jar, so a Cloudflare clearance cookie solved in the WebView never reached a JavaScript
source's requests — the bypass was structurally incapable of working on that backend. Re-run the sweep before
treating any of those five as an external block; the numbers above were measured with that defect in place.
...
| "Emitting empty preferences instead means … a flow that is still alive to deliver the next write" | `catch` runs its block and then **completes the flow**. Emitting a fallback there produces one value and ends, so the setting stayed pinned at its startup default for the rest of the process — exactly the bug the change was written to fix, now wearing a fallback value. `retryWhen` is the operator that resubscribes; the two are indistinguishable from the emitted value alone, which is why the test collects *two* emissions (#1289) |
| "The retry carries the clearance cookie automatically, because the cookie jar and the WebView share Android's `CookieManager`" | The shared client set **no** `CookieJar` at all, so OkHttp used `NO_COOKIES`. Two other files asserted the same jar. Only APK extensions had one — `NetworkHelper` layers its own on top — so the one path likely to be checked by hand was the one path that worked, while JavaScript sources, page images and OPDS all ran without cookies and the Cloudflare bypass did nothing for them (#1285) |
...
4. **JavaScript source failures** — check the site with `curl` first; most are Cloudflare, a moved domain, or the site being down. Then check, in order: is `MProvider` defined (a missing global fails the whole script, not one method); is the source reading a preference nobody set (declared defaults come from `getSourcePreferences()`); did a selector path leak a document handle. **A repeating Cloudflare challenge is now a cookie question**: the shared client carries `WebViewCookieJar` (#1285), so clearance obtained in the WebView reaches OkHttp — if it does not, check that jar before the script. Reproduce with `tools/js-prelude-harness/`. *(APK loader failures — ClassLoader, permissions, interface mismatch — are a separate diagnosis path in `core/extension`; both backends are live, so identify which one the source came from before debugging.)*
...
- **Do not give `JsHttpBridge` back the shared client's cookie jar.** It sets `CookieJar.NO_COOKIES` and attaches cookies per hop instead, scoped to the requesting source's registrable domain (`ownedBySource`). A `CookieJar` sees only a URL, so it cannot tell which source is asking — and the bridge lets a source request any public host by design. Inheriting the jar lets a hostile source read the user's session on any site they have signed into through the WebView, tracker sign-in included. The scoping rule is pinned by `CookieScopePolicyTest`; note it is deliberately *looser* than `isOriginChange` (a subdomain is owned), because those two answer different questions.
